...
export function useComponentPrinter() {
  ...
  const triggerExport = useReactToPrint({
    ...
    print: async (printIframe) => {
      ...
      try {
        canvas = await toCanvas(html, options.htmlToImage);
      } catch (error) {
        console.error("Failed to render canvas for export", error);
        throw error;
      }
  ...
  const runExport = React.useCallback(
    async (format: ExportFormat) => {
      ...
      try {
        if (!triggerExport) {
          throw new Error("Export handler is not ready");
        }
  ...
  const handleDownloadPdf = React.useCallback(() => {
    runExport("pdf").catch((error) => {
      console.error("Failed to export carousel as PDF", error);
    });
  }, [runExport]);

  const handleDownloadPng = React.useCallback(() => {
    runExport("png").catch((error) => {
      console.error("Failed to export carousel as PNG", error);
    });
  }, [runExport]);
